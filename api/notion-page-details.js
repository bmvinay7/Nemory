// Serverless function to handle Notion page details API calls
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Sanitize pageId (UUID format)
    const sanitizedPageId = pageId.replace(/[^a-f0-9-]/gi, '');

    // Make the actual request to Notion API
    const response = await fetch(`https://api.notion.com/v1/pages/${sanitizedPageId}`, {
      method: 'GET',
      headers: {
    ...
    console.error('Notion page details error:', error);
    try {
      return res.status(500).json({ 
        error: 'Failed to fetch Notion page details',
        details: error.message 
      });
    } catch (_) {
      res.statusCode = 500;
      res.end('Failed to fetch Notion page details');
    }
  }
}
